...
const PREFIX = 'БРАТИШКИН СМОТРИТ — ';
...
function updateTitles(root = document) {
  ...
  const watchTitle = root.querySelector(
    'ytd-watch-metadata h1 yt-formatted-string'
  );
  if (watchTitle) prefixTitle(watchTitle);
}
...
window.addEventListener('yt-navigate-finish', () => updateTitles());
*/
